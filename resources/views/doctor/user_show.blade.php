...
<body>
   ...
   <!--Sub-Header-->
   <div id="subheader">صفحة التحكم > المستخدمين</div>
   <!---->
   <section>
      <h1 class="title_table">مستخدمين الموقع</h1>
               ...
               <td>
                  <form method="post" action="/user/{{$user->id}}">
                     ...
                     <select name="role" onchange="this.form.submit()">
                        <option value="1" {{($user->role ==1)?'selected':null}}> مدير</option>
                        <option value="2" {{($user->role ==2)?'selected':null}}>طبيب</option>
                        <option value="3" {{($user->role ==3)?'selected':null}}>السكرتاريا</option>
                        <option value="4" {{($user->role ==4)?'selected':null}}>مستخدم</option>
                     </select>
                  </form>
               </td>
               <td>{{$user->email}}</td>
